...
This is a good time to open `index.html` again (or reload if you still have it loan your browser). It should look the same as before. Type a todo into the text field
and hit `return`.
...
Todos.StatsView = Em.View.extend({
  ...
  remainingString: function() {
    ...
    return remaining + (remaining === 1 ? " in your browser). It should look the same as before. Type a todo into the text field
and hit `return`.
...
Todos.StatsView = Em.View.extend({
  ...
  remainingString: function() {
    ...
    return remaining + (remaining === 1 ? " ining')
});
```
...
control inside the HTML for `StatsVid
s shown here:
