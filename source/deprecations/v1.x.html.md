...
It may even be possible to move the change to `init`, where the value can only be set once.
...
Don't see a `set` in your `didInsertElement`? It may be in `didUpdate` or `didRender`. Due to a bug, these run during the same process.

Still can't find it? One of your computed properties may be calling `set`. Or one of your child components may be setting one of its attributes and it is being propagated upward to this component.
...
```js
  didInsertElement() {
    this._super(...arguments);
    run.scheduleOnce('afterRender', this, this.measureAndSet);
  },
