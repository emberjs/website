...
With `ember-cli@3.4.0-beta.2` you now have the ability to enable the live reloading server and normal app server to share a **single port**. With the current state of Ember CLI it opens two HTTP servers - one for the assets and one to support live reloading on file changes. Until now they have had two different ports, but now they’re all in one port.
