...
HTMLBars is still actively evolving its API and is not 1.0.0 just yet, so to ensure that the templates are compiled compatibly with your Ember version we have updated the Ember build system to generate a `ember-template-compiler.js` alongside every build of Ember.  This template compiler can be used either server-side for precompilation or in the browser to compile templates on the fly. If you do not need to compile templates at runtime (on the client) you do not have to include a runtime (reducing the total size requirement for a production payload). This is a departure from previous versions, where you would always include either `handlebars.js` or `handlebars.runtime.js`.
...
You can use this template compiler directly (without additional requirements):
...
As you will notice, it is nearly identical to how this worked in prior versions.  The main change is the usage of the local `bower_components/ember/ember-template-compiler.js` file (as opposed to the NPM module).

### Client Side Compilation

Generally speaking, you should avoid compiling templates on the client, but there are times that you cannot avoid this (live demo apps like JSBin might be one example).
...
* Others? Would love PR's to add others (we are certain they exist).
