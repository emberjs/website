...
Invoking components via angle brackets was already available, the novelty is that you can now use this syntax with for single word components as well.

For example, the pollyfil supports components names such as `<Button>`, `<Modal>` and `<Tab>`.
